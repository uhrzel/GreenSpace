...
.selectedTopic { position:absolute; left:0px; top:-60px; height: 0px; overflow: hidden; padding: 22px 30px; width:100%; background:red; z-index:99 }
        ...
        ***********************************-->
        ...
        <select id="categorySelect"  class="form-control w-100" name="category" style="width: 100%;color:#19181a" required onchange="this.form.submit()">
           <option value="" selected disabled>
    <?php echo $selcategory == 'Select category' ? 'Select category' : 'Selected category : ' . $selcategory; ?>
</option>
            ...
            <option value="Seek Advice">Seek Advice</option>
                                         ...
                                         <hr style="background: #FFFFFF; width: 92%;margin: 20px auto;margin-bottom: 0px;">
                                    <div class="card-body">
...
// Add WHERE clause if category is not 'All'
if ($category !== 'All') {
    $query .= " WHERE af.category = '$category'";
}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo "
    <div class='blog-author'  data-forum-id='$forum_ID' style='background: #F9F9F9'>
        <div class='media align-items-center'>
            <img src='../homepage/$user_img' alt=''>
            <div class='media-body'>
                <div style='display:flex;justify-content: space-between;'>
                    <div style='color: #19181a'>$user_name</div>
                    <div style='color: #19181a'>" . date('F j, Y', strtotime($date_posted)) . " • $topicrepliescount Replies</div>
                </div>
                ...
                <div style='margin:0px;display:flex;justify-content: space-between;'>
                    ...
                    <div class='delete-btn' data-forum-id='$forum_ID' style='color:#f1f1f1;border-radius:5px;background:red;padding:5px;font-size:.8em;cursor:pointer;'>Delete</div>
                </div>
            </div>
        </div>
    </div>";
...
$(document).ready(function() {
    ...
    $('.blog-author').click(function() {
        var forum_ID = $(this).data('forum-id');

        var form = $('<form action=\"selectedForum.php\" method=\"post\">' +
...
<style>
    ...
    .blog-author2 > div { font-size:.8em }
     ...
     .blog-author2:hover { border:2px solid #007A64; }
...
include 'db.php';
    ...
    $query = "SELECT af.category, af.forum_ID, af.title, af.replyTo, af.user_ID, af.content, af.date_posted, u.user_img, u.user_name, af.category
              FROM allforums AS af
              INNER JOIN user AS u ON af.user_ID = u.user_ID
              WHERE (replyTo IS NOT NULL OR replyTo != '')
              ";
...
$newusername = $row_user['user_name'];
        ...
        echo "
        <div class='blog-author2' data-forum-id='$replyTo' style='background: #F9F9F9'>
            <div class='media align-items-center'>
                <img src='../homepage/$user_img' alt=''>
                <div class='media-body'>
                    <div style='display:flex;justify-content: space-between;'>
